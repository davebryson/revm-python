...
///
/// Abi utilities to parse abi and extract contract information used in the
...
impl From<&str> for ContractInfo {
    fn from(abi: &str) -> Self {
        ...
        let (cp, fns) = build_contract_info(co.abi);
        Self {
            functions: fns,
...
fn build_contract_info(abi: JsonAbi) -> (Option<Vec<String>>, Vec<ContractFunction>) {
